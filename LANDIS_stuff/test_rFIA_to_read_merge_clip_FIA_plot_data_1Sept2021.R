...
#**Is this an action I need to take that isn't represented in the code??**
#**So far, the tpa dataframe reflects this. 
#**Or is this comment talking about the OG FIA data that was downloaded (in English units)?
...
agb <- agb %>% 
  dplyr::mutate(sppcode = stringi::stri_extract_all_words(agb$SCIENTIFIC_NAME) %>% 
           purrr::map(str_sub, 1, 4)  %>% 
           purrr::map_chr(paste, collapse = "") %>% purrr::map(tolower) %>% unlist())

tpa <- tpa %>% 
  mutate(sppcode = stringi::stri_extract_all_words(tpa$SCIENTIFIC_NAME) %>% 
           map(str_sub, 1, 4)  %>% 
           map_chr(paste, collapse = "") %>% map(tolower) %>% unlist())
#once again- this one does work OK.
#SO I will comment out the below code that's doing the same thing a dif way.
